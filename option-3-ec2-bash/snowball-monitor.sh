#!/bin/bash
# multi-snowball-monitor.sh 

# ===============================================
# CONFIGURATION - UPDATE FOR YOUR ENVIRONMENT
# ===============================================

# Array of Snowball device IPs to monitor
SNOWBALL_DEVICES=(
    "10.0.1.100"
    "10.0.1.101"
    "10.0.1.102"
    # Add more IPs as needed, one per line
)

SNOWBALL_PORT="8443"               # Usually 8443 for Snowball
TIMEOUT="5"                        # Connection timeout in seconds
SNS_TOPIC="YOUR-SNS-TOPIC-ARN"    # Your SNS topic ARN

# State file for tracking previous status (for smart alerting)
...
# Note: Removed 'set -e' so script continues checking all devices even if some fail
...
declare -A device_status  # Associative array to track each device status
...
send_device_metric() {
    ...
    if aws cloudwatch put-metric-data \
        ...
        --value "$metric_value" \
        --unit "Count" \
        --dimensions "DeviceIP=$device_ip"; then
...
send_alert_if_changed() {
    ...
    # Get current state as comma-separated string
    local current_state
...
main() {
    ...
    # Check each device
    for device_ip in "${SNOWBALL_DEVICES[@]}"; do
        # Check connectivity (don't exit on failure)
        check_device_connectivity "$device_ip" || true
        
        # Send individual device metric
